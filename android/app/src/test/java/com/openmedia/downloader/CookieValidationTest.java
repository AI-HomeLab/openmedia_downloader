...
public class CookieValidationTest {
    ...
    @Test
    public void siteGuardBlocksWrongSiteFile() {
        // X 的檔貼進 YouTube 格：matchesSite 先擋（save 內同一個判斷；
        // save() 本體要 Context，存檔路徑由 connected CookieStoreTest 覆蓋）。
        String xFile = "# Netscape HTTP Cookie File\n.x.com\tTRUE\t/\tTRUE\t1\tauth_token\tXXX\n";
        assertTrue(!CookieStore.matchesSite(xFile, "youtube"));
    }

    @Test
    public void matchesSite() {
        String xFile = "# Netscape HTTP Cookie File\n.x.com\tTRUE\t/\tTRUE\t1\ta\tb\n";
        assertTrue(CookieStore.matchesSite(xFile, "twitter"));
        assertTrue(!CookieStore.matchesSite(xFile, "youtube"));
        // 混合多站匯出：有該站的就放行。
        assertTrue(CookieStore.matchesSite(
                xFile + ".youtube.com\tTRUE\t/\tTRUE\t1\tc\td\n", "youtube"));
        // google.com 算 YouTube 的（登入態常在那）。
        assertTrue(CookieStore.matchesSite(
                "# Netscape HTTP Cookie File\n.google.com\tTRUE\t/\tTRUE\t1\tNID\tX\n",
                "youtube"));
        assertTrue(!CookieStore.matchesSite("hello world", "youtube"));
    }
