...
public class CookieStoreTest {
    ...
    public void sitesAreIndependent() throws Exception {
        ...
        // 一站一檔：B 站格只收含 bilibili 網域的內容（YouTube 的檔會被擋）。
        String biliFake =
                "# Netscape HTTP Cookie File\n.bilibili.com\tTRUE\t/\tTRUE\t1\tSESSDATA\tABC123\n";
        store.save(CookieStore.BILIBILI, biliFake);
        assertTrue(store.has(CookieStore.BILIBILI));
        assertFalse(store.has(CookieStore.TWITTER));
        try {
            store.save(CookieStore.BILIBILI, FAKE);
            throw new AssertionError("錯站存檔應拒絕");
        } catch (DownloadException e) {
            assertEquals(DownloadError.EXTRACT, e.getCode());
        }
        assertEquals(biliFake, store.load(CookieStore.BILIBILI));
        store.clear(CookieStore.BILIBILI);
    }
